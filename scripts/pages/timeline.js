...
(function () {
  ...
  function ReportKpis(props) {
    ...
    // JSON populates but the live Aurora/extraction path historically did not — so
    // every real timeline rendered a hard 0 (UCPK2: 21 recordings in the DB, "0" shown).
    // org-api now counts them live off the `recordings` table (recordings_processed +
    // duration_seconds). Gate on the FIELD, not on the metadata block: the live path
    // always emits a `_report_metadata` ({source, version}), so a block-level check is
    // true even when the counts are missing and would show the misleading 0 again.
    // Absent field -> em dash ("metric unavailable"), which is what a day served by an
    // older org-api, or the reindex builder, still gets. Topics + Safety are derived
    // from the live report and are accurate regardless.
