  ...
  function QualityCreateModal(props) {
    ...
    React.useEffect(function () {
      ...
      window.FS.api.sites.getSites().then(function (res) {
        ...
        if (cancelled) return;
        console.error('[QualityCreateModal] getSites failed', err);
        setSitesList([]);
        setSitesError(true);
      });
    ...
    return React.createElement('form', {
      ...
      console.error('[QualityCreateModal] render failed', renderErr);
      return ErrorBanner
        ? React.createElement(ErrorBanner, {
    ...
    if (ModalOverlay) {
      /* T5 fix — ModalOverlay only paints + becomes interactive when
         ...
         mounted while the page's showCreate state is true (quality.js),
