...
(function () {
  ...
  async function getTranscripts(opts) {
    ...
    if (!window.FS.api.useMocks) {
      ...
      /* authority flip (pipeline plan 2026-07-14): the legacy /transcripts
         ...
         the transcript S3 object exists. Mirrors timeline.js's aurora gate
         (fetchTimeline, api/timeline.js) verbatim; 'aurora' only takes
         effect when orgBaseUrl is non-empty (kill switch). */
      if (window.FS.api.timelineSource === 'aurora' && window.FS.api.orgBaseUrl) {
        return window.FS.api.orgRequest('/transcripts', { params: params });
      }
      return window.FS.api.request('/transcripts', { params: params });
    }
